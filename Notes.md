...
### Motor
* Each muscle fiber is innervated by 1 motor neuron, each motor neuron can innervate >1 muscle fiber.
...
* In the legs:
  ...
  * Sensory receptor for stretch's axon tocrus = la afferent.
  * In CNS, each mn receives input from several la afferents and each la afferents has inputs to several alpha mns.
  * Each single epsps from a la afferent is sub-threshold, but can bring a mn to threshold if la afferent fires a burst of spikes ad epsps sum temporally and/or several la afferents are activated and epsps sum by both spacial and temporal summation.
  * Strength of a muscle's contraction can be increased by:
    * imcreasing amt of contraction by mn firing more ap on to muscle fibers
    * and/or  increasing the number of motor units in the muscle that are activated
  * motorneurons can e different sizes  
  ...
  * suppose you want to lift an item - activate a few small mns, get a small contraction. Could do job but if not, bring in more mn units
    * brings in large mn which cause more fibers to contract.
    * a graded strength to the contraction.
  * why does the small motorneurons get activated first?
    * being small, the la afferent synapses are closer to the SIZ so epsps are larger when they reach that place (length constant is effectively larger)
    * so its easier for an epsps generated by input from a la afferent to reach threshold in a small mn than a large mn. Less distance for it to travel.
